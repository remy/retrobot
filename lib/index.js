...
function handleReaction(message) {
  ...
  retrospective.workedwell.forEach(r => {
...
function command(message) {
  ...
  if (!aliases[cmd]) {
    return reply(message, ':thinking_face:☔️ nope, does not compute. The upshot though, I do support `start`, `stop` and `summary` try those instead ⛅️');
  }

  if (cmd === 'summary') {
    const topWorkedWell = retrospective.workedwell.filter(_ => !!_.plusone).sort((a, b) => {
      return a.plusone < b.plusone ? 1 : -1;
    }).slice(0, 3).map(r => {
      return ':+1:'.repeat(r.plusone) + ' ' + r.text.replace(/^\d+\.\s/, '');
    });
    topWorkedWell.unshift(`*Below is the worked well summary, good work team :clapping: :snyk: *`);

    const topNeedsWork = retrospective.needswork.filter(_ => !!_.plusone).sort((a, b) => {
      return a.plusone < b.plusone ? 1 : -1;
    }).slice(0, 5).map(r => {
      return ':+1:'.repeat(r.plusone) + ' ' + r.text.replace(/^\d+\.\s/, '');
    });

    topNeedsWork.unshift(`*Below is the needs work summary, always improving :chart_with_upwards_trend: *`);

    const summary = topWorkedWell.concat(topNeedsWork);

    const source = { channel: message.channel || bot.self.channel };

    flushMessages(source, summary);
...
function flushMessages(source, messages, messageType) {
  reply(source, messages.shift(), (error, msg) => {
    if (messageType === 'needsWork' && msg) {
      retrospective.needswork.push(msg.message);
    }

    if (messageType === 'workedWell' && msg) {
      retrospective.workedwell.push(msg.message);
    }
    ...
    if (msg && msg.message.text.includes('"worked well"')) {
      messageType = 'workedWell';
    }

    if (messages.length) {
      flushMessages(source, messages, messageType);
    }
  });
