...
function captureRetrospective(message) {
  ...
  lines.map(_ => _.trim()).forEach(line => {
    ...
    let type = types[line[0]];
    if (type) {
      return retrospective[type].push(line.slice(1).trim());
    }
...
function notInRetro(message) {
  let text = 'The retrospective isn\'t runnning at the moment. Here, have ' +
...
function command(message) {
  ...
  if (aliases[cmd] === 'chicken') {
    return reply(message, '🍗 oh…hell yes :nandos:');
  }
    ...
    slack.channels.info({ token, channel }, (error, data) => {
      ...
      members.filter(user => user !== bot.self.id).map(user => {
        slack.im.open({ token, user }, (error, data) => {
          ...
          console.log(`inviting ${user} to retro`);
