...
function command(message) {
  ...
  if (!aliases[cmd]) {
    return reply(message,
      ':thinking_face:☔️ nope, does not compute. The upshot though, I do support `start` and `stop`, try those instead ⛅️'
    );
  }
  ...
  if (cmd === 'summary') {
    ...
    const source = {
      channel: message.channel || bot.self.channel
    };
  ...
  if (aliases[cmd] === start) {
    if (inRetro) {
      return reply(message,
        'Another channel is currently using your friendly neighbourhood retrobot. Please try again later.'
      );
    }
    ...
    getRetroParticipants(token, channel, (error, members) => {
      if (error) {
        reply(message,
          'Sorry, I failed to contact the participants of this retro... ' +
          error);
      }
          ...
          slack.users.getPresence({
            ...
            slack.im.open({
              ...
              reply({
                ...
                '👂 Ping me your points for the retrospective' +
...
function stopRetro(message) {
  if (inRetro === false) {
    return reply(message,
      'The retrospective finished already. I don\'t have a historical record of the retro to print here either. Sorry. Have some tea 🍵'
    );
  }
  if (inRetro !== message.channel) {
    return reply(message,
      'Another channel is currently running a retrospective, you\'ll need to wait for them to finish.'
    )
  }
  inRetro = false;
  ...
  let msg =
    `⏰ time's up! The retrospective is over, head on back to <#${channel}>`;
  ...
  setTimeout(flushRetrospective.bind(null, retrospective), Math.random() * 3000 +
    1000 | 0);
}

function flushRetrospective(retrospective) {
  const plus = shuffleArray(Object.keys(retrospective.plus).map(_ =>
    retrospective.plus[_]));
  const minus = shuffleArray(Object.keys(retrospective.minus).map(_ =>
    retrospective.minus[_]));

  debug('flushing retrospective (plus: %s, minus: %s)', plus.length, minus.length);

  const source = {
    channel: channel || bot.self.channel
  };
