...
function captureRetrospective(message) {
  ...
  if (message.edited) {
    // find all the messages with the `ts` and remove them
    for (let i = 0; i < retrospective.plus.length; i++) {
      const r = retrospective.plus[i];
      if (r && r.ts === message.ts) {
        retrospective.plus.splice(i, 1);
        i--;
      }
    }

    for (let i = 0; i < retrospective.minus.length; i++) {
      const r = retrospective.plus[i];
      if (r && r.ts === message.ts) {
        retrospective.minus.splice(i, 1);
        i--;
      }
    }
  ...
  lines.map(_ => _.trim()).forEach(line => {
    ...
    if (type) {
      return retrospective[type].push({
        user: message.user,
        ts: message.ts,
        line: line.slice(1).trim() + ` – <@${message.user}>`,
      });
    }
...
function flushMessages(source, messages, track = false) {
  reply(source, messages.shift(), (error, msg) => {
    ...
    if (msg.message.text.includes('needs work')) {
      track = true;
    }
    if (messages.length) {
