...
function command(message) {
  ...
  if (!aliases[cmd]) {
    return reply(message, ':thinking_face:☔️ nope, does not compute. I do support `start` and `stop`, try those instead ⛅️');
  }
        ...
        slack.dnd.info({ token, user }, (error, dnd) => {
          ...
          slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            slack.im.open({ token, user }, (error, data) => {
              ...
              reply({ channel: dms[user] }, '👂 Tell me about your sprint' +
                '\nStart lines with `+` for "worked well" and `-` for "needs ' +
                'improvement". When we\'re done, we can vote on all the ideas and talk about it.');
            });
          });
...
function flushRetrospective(retrospective) {
  ...
  messages.push(`\n*Please go ahead and use the 👍 reaction to indicate that you think a particular item should become a talking point.*\n\nSee you after the next sprint 👋🔌`);
